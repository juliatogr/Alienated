...
void Image::drawImage(const Image& img, int x, int y, int w, int h, Area rect)
{
	if (x > (int)width || y > (int)height || (x + (int)rect.w) < 0 || (y + (int)rect.h) < 0)
		return; //outside

	rect.x = clamp(rect.x, 0, (int)img.width);
	rect.y = clamp(rect.y, 0, (int)img.height);
	rect.w = clamp(rect.w, 0, (int)img.width - rect.x);
	rect.h = clamp(rect.h, 0, (int)img.height - rect.y);
	int startx = clamp(x, 0, (int)width);
	int starty = clamp(y, 0, (int)height);
	int endx = clamp(x + rect.w, 0, (int)width);
	int endy = clamp(y + rect.h, 0, (int)height);

	//iterate 
	for (int i = startx; i < endx; ++i)
		for (int j = starty; j < endy; ++j)
		{
			const Color& c = img.getPixel(i - x + rect.x, j - y + rect.y);
			if (c.a == 0)
				continue;
			int pos = j * width + i;
			if (c.a == 255)
