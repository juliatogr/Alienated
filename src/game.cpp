...
Game::Game(int window_width, int window_height, SDL_Window* window)
{
	...
	world->tileset.loadTGA("data/tileset.tga"); //example to load an sprite
	world->loadGameMap("data/mymap.map");
	world->spawnEnemies();

	
	InitStages(stages);
	current_stage = STAGE_ID::INTRO;
	stages[STAGE_ID::INTRO]->activateButtons();
	world->player = new Player();
	world->tutplayer = new Player();
	world->bg = new Background();
	world->bg->InitLayers();
...
void Game::render(void)
{
	...
	GetCurrentStage(stages, current_stage)->render(framebuffer, world, time);
	showFramebuffer(&framebuffer);
}

void Game::update(double seconds_elapsed)
{
 	stages[current_stage]->update(seconds_elapsed, world, stages, current_stage, must_exit);
}
